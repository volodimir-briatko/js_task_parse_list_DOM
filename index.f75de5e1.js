...
function sortList() {
    ...
    document.querySelectorAll("ul li").forEach((el)=>{
        list1.push({
            ...
            salary: Number(el.getAttribute("data-salary").replaceAll(",", "").replaceAll("$", "")),
            age: el.getAttribute("data-age")
        });
    });
    list1 = list1.sort((a, b)=>b.salary - a.salary);
    document.querySelector("ul").innerHTML = "";
    for (const i of list1)document.querySelector("ul").insertAdjacentHTML("beforeend", `<li data-position="${i.position}"
